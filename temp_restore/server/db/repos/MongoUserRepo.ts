...
export class MongoUserRepo implements IUserRepo {
  async findById(id: string): Promise<User | null> {
    ...
    if (!d) return null;
    return new User(d._id.toString(), d.name||'', d.email||'', d.phone||'', d.role||'', d.password, d.sellerId);
  }
  ...
  async findAll(): Promise<User[]> {
    ...
    return docs.map(d => new User(d._id.toString(), d.name||'', d.email||'', d.phone||'', d.role||'', d.password, d.sellerId));
  }
